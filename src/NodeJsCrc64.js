 ...
 * - https://github.com/torvalds/linux/blob/master/lib/gen_crc64table.c
 * - https://www.sunshine2k.de/coding/javascript/crc/crc_js.html
 */

export const CRC64_NVME_REVERSED_TABLE = [
...
export const crc64_NVMe = (data) => {
  let crc = BigInt("0xFFFFFFFFFFFFFFFF");

  // @ts-ignore
  for (let i = 0; i < data.length; i++) {
    const byte = BigInt(data[i]);
    const tableIndex = Number((crc ^ byte) & BigInt(0xff));
    crc = (crc >> BigInt(8)) ^ CRC64_NVME_REVERSED_TABLE[tableIndex];
  }

  crc = crc ^ BigInt("0xFFFFFFFFFFFFFFFF");

  // return the CRC64 checksum as a 16-character hexadecimal string
